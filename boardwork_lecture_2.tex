...
Before proceeding, first notice that although there are technically 3 players in this game, there are really only 2 interesting players. Does anyone want to guess which of the players is not interesting (and volunteer a reason for their guess)? The worker is not really interesting. This is because the worker is mechanical: they take the job that pays the most. Once we realize this, we can just pretend like there are two players: the two firms. The two players get the worker if they post a higher wage than the other, get a 50-50 shot at the worker if they tie, and get 0 if they post too low a wage.
...
Well, no: either firm could offer some wage slightly more than 10 (say, 11). Then they get the worker for sure, and they get a profit of $1*(20-11)=9>5$, clearly much better than before. This is a profitable deviation. What if we repeat this? Suppose firm 1 now offers 11, and firm 2 offers 10. If they both do this, firm 1 gets profit of 9 and firm 2 gets 0. Who has a profitable deviation? Well, firm 2 can now out-bid 1 and offer slightly more than 11 (say, 12). Now firm 2 gets the worker for sure and gets profit $1*(20-12)=8>0$. We can continue repeat this process for any wages that are less than $p=20$.

This loose discussion suggests a guess: what if both firms offer a wage of $20$? Perhaps this is a Nash equilibrium (and a solution for the purposes of this class). To be general, let's guess that both firms play $w_1=w_2=p$.
\begin{itemize}
    ...
    \item First note that when $w_1=w_2=p$ profit for both is 0 because whichever firm gets the worker pays out exactly their productivity so: $0.5*(p-w_i)=0$.
    \item Suppose one firm attempts to outbid by offering some higher wage $w_i'>p$ while the other stays. Does the deviating firm gain from doing this?
    \item No: the deviator wins the worker for sure, but now the worker is being paid more than their productivity, so the firm makes negative profit: $1*(p-w_i')<0$.
    \item What if one firm tries to undercut with a lower wage: $w_i'<p$? Do they gain?
    \item No: the deviating firm now misses out on the worker for sure, and continues to get a profit of 0: $0*(w_i'-p)=0$
    \item Thus $w_1=w_2=p$ is an NE.
\end{itemize}

For the purposes of this class, this is enough for a complete answer, as you have just proven that $w_1=w_2=p$ is a Nash Equilibrium. Now, you may wonder, is this the only Nash Equilibrium? It turns out it is. Showing this requires only a bit more work. We will prove this by contradiction.
...
\section{Company Call List - Static}
    ...
    \begin{enumerate}
        \item[a.] Write down the utility-maximization problem for worker $i$. We can focus on some generic worker $i$ because both workers are the same. This is called solving by symmetry.\\
        \textbf{Solution)}\\
        $$
        ...
        To find the optimal choices, we take the FOC of the utility expression. This is the partial derivative with respect to $q_i$:
        $$
        ...
        \item[d.] I will not require this on exams but we can also prove that this NE is the unique (or only) pure strategy Nash Equilibrium using a bit of calculus.\\
        \textbf{Solution)}\\
        ...
        \end{align*}
        Note that the equation (1) includes $2q_2$ rather than just $q_2$ (same for equation (2)). This can be interpreted as the social planner taking  into account externalities when choosing $q_1$ and $q_2$. This does not occur in the Nash Equilibrium we solved for earlier. (Why?) Solving the above system, we get:
        $$
        ...
        q_1^{**}+q_2^{**}=60.
        $$
        Among these, one natural solution would be equal division of the calls: $q_1^{**}=q_2^{**}=30$.\\
        Then accordingly, the utility for each worker under the social planner's equal division solution is:
        \begin{align*}
            &u_1(30)=(120-30-30)30=1800\\
            &u_2(30)=1800.
        \end{align*}
        The utility from the Nash equilibrium is lower:
        $$
        u_1(q_1^*)=u_2(q_2^*)=u_1(40)=(120-40-40)40=1600.
        $$
        Thus, we see that both workers would be strictly better off if they could commit to only making 30 calls each. A firm could make this happen by having a supervisor who assigns calls.
...
Suppose everything about that problem is the same, except that player 1 moves first and then player 2 moves.
    \begin{enumerate}
        ...
        \item[c.] Compute profit for worker 1 and worker 2. Some games are said to exhibit a ``first-mover advantage." Does this game?
         ...
         Since $u_1>u_2$ and worker 1 moved first this game has a first-mover advantage. This is because the first-mover (worker 1) can call more people knowing that worker 2 will compensate by calling less people. We see that relative to the static problem, worker 1 is calling more  while worker 2 is calling less.
