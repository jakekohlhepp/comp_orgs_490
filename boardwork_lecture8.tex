...
However, this step does not make sense in this paper because they are running an experiment and ``shocking" the meaning workers get from the job. So they do not assume the firm has extracted all surplus. Instead, they observe that the profit of the firm will be the fraction of out put the firm keeps ($1-\beta_{ij}$) less an exogenous (fixed) salary $\alpha_{ij}$. So they skip the middle step and jump straight to profit maximization. The firm's profit is:
\[\max_{b_{ij}} \theta_i (1-\beta_{ij}) e_{ij} - \alpha_{ij} \]
Plugging in $e_{ij}$:
\[\max_{b_{ij}} \theta_i (1-\beta_{ij}) \beta_{ij}\theta_i (1+\lambda_i m_{ij} ) - \alpha_{ij}= \max_{b_{ij}} \theta_i (\beta_{ij}-\beta_{ij}^2) \beta_{ij}\theta_i (1+\lambda_i m_{ij} ) - \alpha_{ij}\]
Taking the FOC:
