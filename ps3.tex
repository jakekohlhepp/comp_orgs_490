...
\item Propose strategies for the firm and the worker. Explain the strategies in your own words.




\item Argue that the low wage ($w_L$) must be below a certain value. Propose a value for the low wage.

\item Explain the states that are relevant to payoffs under your guessed strategies and assign them labels. Denote the firm and worker's payoffs under any action today and any state (and the proposed strategies tomorrow) as $\Pi(a,s), U(a,s)$.


\item Compute the payoffs from not deviating from your guessed strategies for the firm and the worker.



\item What are the two classes of one-shot deviations for the firm? Construct an inequality for each under which the firm has no incentive to deviate. Simplify both and show that one is always satisfied.


\item What are the two classes of one-shot deviations for the worker? Construct an inequality for each under which the worker has no incentive to deviate. Simplify both and show that one is always satisfied.



   \item Combine your past work to construct an inequality under which there exists a relational contract which sustains high effort.


    \item For this and the next question,    assume $w_H$ is the lowest possible wage which sustains high effort.  Suppose $\delta = 0.4, v=3, \bar u = 1, c=1$. Is the relational contract we derived profitable for the firm?
