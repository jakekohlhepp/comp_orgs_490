...
\title[diss]{Lecture 15: Relational Contracts} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\begin{wideitemize}
    ...
    \item Interpretation 1: If I have a higher $\delta$, I am more patient.
    \item Interpretation 2: $\delta$ is the chance we meet again tomorrow.
    \begin{wideitemize}
...
\begin{frame}{Model}
\begin{wideitemize}
    \item A firm and a worker both have discount rate $\delta\in [0,1)$ and interact for many periods ($t=1,...,\infty$)
    \item At each period $t$ the following occur:
    \begin{wideitemize}
        \item First, the firm offers a flat wage $w_t$
        \item Second, the worker chooses one of three options: reject the job (R), low effort (L), or high effort (H).
        \begin{wideitemize}
            \item Formally: $e_t \in \{R,L,H\}$
        \end{wideitemize}
    \end{wideitemize}
    \item High effort has cost $c>0$, low effort has cost 0.
    \item High effort yields revenue $v>0$, low effort yields revenue 0.
    \item Rejection yields an outside option for the firm of 0 and $\bar u$ for the worker.
    \item Assume the firm wants to motivate high effort: $v-c>0$. Assume the worker chooses high effort over low effort if indifferent, and low effort over rejecting the job if indifferent.
\end{wideitemize}
...
\begin{wideitemize}
    ...
    \end{wideitemize}
    \item For more information on infinitely repeated games see the slides from my game theory course.
\end{wideitemize}
    
\end{frame}


\begin{frame}{Step 1: Guessing Strategies}
\begin{block}{Worker Strategy}
 Choose $e_t=H$ if $w_{\tau}\geq w_h$ for all $\tau \leq t$. Otherwise, choose $e_t=R$.
\end{block}
\textbf{In words:} exert high effort as long as the high wage is offered.
\begin{block}{Firm Strategy}
Choose $w_t=w_H$ if $t=1$ or if $t>1$  and $e_{\tau}=H$ for all $\tau < t$. Otherwise, choose $w_t=w_L$.
\end{block}
\textbf{In words:} offer the high wage as long as high effort is provided.
\end{frame}


\begin{frame}{Step 2: What Happens ``On Path"?}

\begin{wideitemize}
    \item The outcome ``on path" is what actually occurs if everyone plays the proposed strategies:
    \begin{enumerate}
        \item At $t=1$, the firm proposes $w_1=w_H$.
        \item Seeing $w_1=w_H$, the worker responds with $e_1=H$.
        \item At $t=2$, and after seeing $e_1=H$, the firm proposes $w_2=w_H$, etc.
    \end{enumerate}
    \item ``On path" the worker is paid $w_H$ forever and exerts high effort forever.
        \item This is distinct from the strategy; a strategy states also what happens when other people play something different than the proposed strategy.
\end{wideitemize}
\end{frame}

\begin{frame}{Step 3: Verify}
\begin{wideitemize}
    \item We now need to verify that our strategy guess forms an equilibrium.
    ...
    \item In general there are infinite alternative strategies.
...
\begin{definition}
    ...
    \begin{wideitemize}
        \item We only need to consider one-shot (single period) deviations.
        \item We only need to each unique situation that happens under the guessed strategy.
    \end{wideitemize}
...
\begin{theorem}
    If $  v \geq  \frac{c}{\delta}+  \bar u$, there is a subgame perfect Nash equilibrium with our guessed strategies where the firm offers a high wage as long as the worker has always exerted high effort, and low wage of $w_L^*=0$ otherwise. The worker exerts high effort as long as the firm has always offered at least $w_H^*$, and the worker does not exert high effort otherwise.
\end{theorem}
    \begin{wideitemize}
...
\begin{wideitemize}
    ...
    \item In this way the value of employment encourages high effort.
    \item Employment is valuable because the worker is paid more than their outside option.
\end{wideitemize}
...
\end{definition}
\begin{wideitemize}
    \item We can think of a worker's productivity as their effort.
    \item On path, the worker exerts high effort and is productive.
    \item If they ever exert low effort (are unproductive) they never work with the firm again.
