...
\title[diss]{Lecture 14: Relational Contracts} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\begin{definition}
    ...
    \begin{wideitemize}
        ...
        \item The one-shot deviation principle means we only need to check for deviations in these two cases.
        \item Further, we only need to consider one-shot (single period) deviations.
    \end{wideitemize}
...
\begin{theorem}
    If $\delta(v-\bar u) \geq c$, there is a subgame perfect Nash equilibrium where the firm offers a wage of $w_H^*=\frac{c}{\delta}+\bar u$ as long as the worker exerts high effort, and a wage of $w_L^*=0$ forever after the worker exerts low effort once.
\end{theorem}
    \begin{wideitemize}
