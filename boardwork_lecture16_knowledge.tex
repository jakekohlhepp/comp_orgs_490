...
\[ U_t = \int_0^\infty w_{t+\tau} e^{-r\tau}d\tau\geq \int_0^\infty (x_t-\theta)^+ e^{-r\tau}d\tau= \frac{(x_t-\theta)^+}{r} \quad \text{for all } t\geq 0 \]
...
\[U_t= \frac{(1-\theta)}{r}e^{-r(T-t)}\]
Suppose the expert did pay strictly more. This means that:
\[U_t= \frac{(1-\theta)}{r}e^{-r(T-t)} > \frac{x_t-\theta}{r}  \]
...
\end{equation}
Now, this equation is true when $t=T$, so we have that:
\begin{equation}
...
\begin{align}
\Pi &= \int_0^T e^{-rt} (x_t-\theta) dt \text{ substitute equation (\ref{eq:unite})}\\
&= T(x_0-\theta)\\
...
Plugging this back in to Equation (\ref{eq:bigt}) gives $x_0^*$:
