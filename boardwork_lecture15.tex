...
We propose the following strategies for the firm and the worker, and subsequently check that the strategies form a sub-game perfect Nash equilibrium using the one-shot deviation principle:
...
The worker compares this to the utility from not deviating which we already calculated:
...
As long as $v\geq  \frac{c}{\delta}+  \bar u$, we can find a relational contract that sustains high effort using our guessed strategy.
