...
We now want to know if there exists a wage scheme that is a partnership that also achieves first-best effort $e^*$. To find out, we suppose for sake of contradiction that there exists a wage scheme $w_i(e)$ which implements first-best effort $e^*$. Given a wage scheme $w_i(e)$, worker $i$ chooses effort based on the FOC:
...
\[ w_i'(y(e^*)) =c_i'(e_i^*)=1 \]
So we must be paying each worker a dollar for their marginal effort. Recall that a partnership by definition is a wage scheme where what is paid out to everyone is equal to the output produced:
\[\sum_{i=1}^N w_i(y(e))=y(e)\]
We now work with this ``budget balance" equation. Take the derivative of both sides of this equation with respect to total output/effort $y(e)$ (not individual effort):
...
This means that the marginal dollar paid to all workers must sum up to 1. But under the first-best, everyone must get the marginal dollar produced: $w_i'(y(e^*))=1$. But if we plug this in, we get a contradiction:
...
Now, is the group bonus we proposed feasible? Is there money to pay for it? Yes! We first consider the case when $e_i=e_i^*$. Remember that first-best effort is defined as the level of effort which solves:
...
Implementing the first-best using group bonuses thus requires credibly destroying or getting rid of output when people slack. In some organizations there is no third party boss or shareholder, so it is hard to credibly commit to destroy output rather than pay out. This provides a reason for the existence of an organization, like a corporation, where ownership and control are separated.
